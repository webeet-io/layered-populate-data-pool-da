...
class WebscrapingDataCleaner:
    """
    Spezialisierte Klasse für Webscraping-Datenbereinigung
    
    Verantwortlichkeiten:
    - Webscraping-Artefakte erkennen und entfernen
    - Null-ähnliche Werte standardisieren
    - Bereinigungsstatistiken verfolgen
    """
    ...
    def _clean_column_artifacts(self, df: pd.DataFrame, column: str) -> Dict[str, Any]:
        ...
        for null_val in self.null_values:
            if null_val == '':  # Handle empty string specially
                count = (df[column] == '').astype("int64").sum()
            else:
                count = (df[column] == null_val).astype("int64").sum()
            
            if count > 0:
                artifacts_found[null_val] = count
                total_cleaned += count
        
        # Replace all artifacts with NaN
        df[column] = df[column].replace(self.null_values, np.nan)
        
        # Clean whitespace and convert empty strings to NaN
        df[column] = df[column].astype(str).str.strip()
        df[column] = df[column].replace('nan', np.nan)  # Handle 'nan' strings from conversion
    ...
    def detect_potential_artifacts(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in df.columns:
            if df[col].dtype == 'object':
                ...
                for null_val in self.null_values:
                    count = (df[col] == null_val).astype("int64").sum()
                    if count > 0:
